...
def gt_by_chom_arm(node_a, node_b):
    ...
    arm as first discriminant"""
    if not node_a:
        return 0
    if not node_b:
        return 1
    discriminants_a = (int(re.split('p|q',node_a.loci)[0]),
    re.search('p|q',node_a.loci)[0], sum(node_a.value))

    discriminants_b = (int(re.split('p|q',node_b.loci)[0]),
    re.search('p|q',node_b.loci)[0], sum(node_b.value))
    
    if discriminants_a>=discriminants_b:
        return 1
    else:
        return 0
...
def read_sec_locations(file_name):
    ...
    with open(file_name) as fh:
        ...
        for line in fh:
            new_node = (line.split("\t")[1], tuple(float(x) 
                for x in re.split('\\(|,|\\)',line)[1:3]))
            if fi:
                to_return = lnk_list(new_node[0],new_node[1])
                fi=0
            else:
                to_return.new_node(new_node[0],new_node[1])
        
    return(to_return)
    

def count_close_reads(sorted_lnk_list, threshold):
    ...
    chrom1 = re.split('p|q',sorted_lnk_list.root.loci)[0]+re.search(
                                    'p|q',sorted_lnk_list.root.loci)[0]
    count_result = lnk_list(chrom1, 0)
    node1 = sorted_lnk_list.root
    while node1.n:
        node2 = node1.n
        chrom2 = re.split('p|q',node2.loci)[0]+re.search('p|q',
                                                        node2.loci)[0]
        dif = 0
        while node2 and chrom1==chrom2 and dif < max_diference_coord_sum:
            ...
            chrom2 = re.split('p|q',node2.loci)[0]+ re.search('p|q',
                                                        node2.loci)[0]
        node1 = node1.n
        chrom1 = re.split('p|q',node1.loci)[0]+re.search('p|q',
                                                        node1.loci)[0]
...
if __name__=="__main__":
    our_data = read_sec_locations(sys.argv[1])
    our_data.start()
    our_data.insert_sort(gt_by_chom_arm)
    counts = count_close_reads(our_data, float(sys.argv[2]))
